...
describe('ConsentEvaluator', () => {
  let evaluator: ConsentEvaluator
  ...
  describe('Platform consent evaluation', () => {
    it('should allow access with valid platform consent', async () => {
      const result = await evaluator.evaluateConsent(
        'client_123',
        'platform',
        null,
        'care'
    ...
    it('should deny access for unsupported purpose at platform level', async () => {
      const result = await evaluator.evaluateConsent(
        ...
        'platform',
        null,
        'research' // Not allowed at platform level
      )
  ...
  describe('Organization consent evaluation', () => {
    ...
    it('should deny access for unsupported purpose at organization level', async () => {
      const result = await evaluator.evaluateConsent(
        ...
        'organization',
        'org_123',
        'QA' // Allowed at platform but not at org level
      )
  ...
  describe('Location consent evaluation', () => {
    ...
    it('should deny access for wrong location', async () => {
      const result = await evaluator.evaluateConsent(
        'client_123',
        'location',
        'location_999', // Different location
        'care'
      )
  ...
  describe('Grace period handling', () => {
    it('should handle grace period logic', async () => {
      ...
      const result = await evaluatorWithGrace.evaluateConsent(
        'client_123',
        'organization',
        'org_123',
        'care'
  ...
  describe('Purpose validation', () => {
    it('should validate purpose against allowed purposes', async () => {
      ...
      const careResult = await evaluator.evaluateConsent(
        'client_123',
        'organization',
        'org_123',
        'care'
      ...
      const billingResult = await evaluator.evaluateConsent(
        'client_123',
        'organization',
        'org_123',
        'billing'
      ...
      const researchResult = await evaluator.evaluateConsent(
        'client_123',
        'organization',
        'org_123',
        'research'
  ...
  describe('Scope-specific consent requirements', () => {
    it('should handle platform vs organization vs location scopes', async () => {
      ...
      // Platform scope
      const platformResult = await evaluator.evaluateConsent(
        clientId, 'platform', null, 'care'
      )
      expect(platformResult.consent_ok).toBe(true)
      expect(platformResult.scope_type).toBe('platform')
      
      // Organization scope
      const orgResult = await evaluator.evaluateConsent(
        clientId, 'organization', 'org_123', 'care'
      )
      expect(orgResult.consent_ok).toBe(true)
      expect(orgResult.scope_type).toBe('organization')
      
      // Location scope
      const locationResult = await evaluator.evaluateConsent(
        clientId, 'location', 'location_456', 'care'
      )
