...
export class NoteContextBuilder extends BaseContextBuilder {
   ...
   */
  private async getNoteInfo(noteId: string): Promise<{
   ...
   */
  private async getNoteAssignmentContext(
    noteId: string,
    userId: string
  ): Promise<{
   ...
   */
  private async getClientAssignmentInfo(
    clientId: string,
    userId: string
  ): Promise<{
    assigned_to_user: boolean
   ...
   */
  private async getNoteOrganizationalContext(
    noteId: string,
    userId: string
  ): Promise<{
    same_org: boolean
   ...
   * Determine access requirements based on note classification
   */
  private determineAccessRequirements(
    noteInfo: {
    ...
    requires_temp_grant: boolean
  } {
    // Confidential notes require temporary grants for non-authors
    const requires_temp_grant = 
      noteInfo.classification === 'confidential' && 
      !isAuthor && 
      action === 'read'
 ...
 */
export function createNoteContextBuilder(): NoteContextBuilder {
